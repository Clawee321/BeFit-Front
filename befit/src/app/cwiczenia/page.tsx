...
import { GetExerciseData, getExerciseDetails } from "@/app/hooks/cwiczeniamain";
import LoadingScene from "@/components/common/LoadingScene";

export default function Ćwiczenia() {
  const [selectedCategory, setSelectedCategory] = useState<Category>("Joga");
  ...
  const {
    ...
    refetch,
  } = useQuery("cwiczeniamain", getExerciseDetails);

  useEffect(() => {
    refetch();
  }, [selectedCategory, refetch]);
  console.log("cwiczeniamain:", exerciseData);
  ...
  const filteredExercises =
    ...
    exerciseData
      ? exerciseData.filter(
          (ćwiczenie) => ćwiczenie.kategoria === selectedCategory
        )
      : [];
    ...
    >
      ...
      <div className="max-w-md mt-12 ml-16 mb-12">
        ...
        >
          ...
          <option value="Ćwiczenia taneczne">Ćwiczenia taneczne</option>
          <option value="Ćwiczenia na brzuch">Ćwiczenia na brzuch</option>
          <option value="Ćwiczenia na pośladki i nogi">
            Ćwiczenia na pośladki i nogi
          </option>
      ...
      <div className="mx-12 mt-12 pb-12">
        <div className="grid grid-cols-2 gap-x-32 gap-y-14">
          ...
          exerciseData ? (
            <>
              {filteredExercises.map((ćwiczenie, index) => (
                <div
                  key={index}
                ...
                >
                  ...
                  >
                    <div className="flex justify-center px-4 pt-4 pb-4">
                      <Image
                        src={`data:image/png;base64,${ćwiczenie.image}`}
                        alt={ćwiczenie.title}
                        width={1280}
                        height={720}
                      />
                    </div>
                    <h2 className="text-2xl font-bold text-black px-4 font-abhaya">
                      {ćwiczenie.title}
                    </h2>
                  </Link>
                </div>
